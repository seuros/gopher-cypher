...
// rawExpr allows embedding a literal Cypher snippet as an Expression.
type rawExpr string

// BuildCypher returns the raw Cypher string without parameterization.
func (r rawExpr) BuildCypher(q *Query) string { return string(r) }

func compileNodes(nodes ...Node) (string, map[string]interface{}) {
	c := NewCompiler()
	c.Compile(nodes...)
	return c.Output(), c.parameters
}

func lit(v interface{}) *LiteralData { return &LiteralData{Value: v} }

func TestOptimizedSnippets(t *testing.T) {
	tests := []struct {
		name     string
		nodes    []Node
		expected string
		...
		{
			name:     "return_param",
			nodes:    []Node{&ReturnNode{Items: []interface{}{lit(1)}}},
			expected: "RETURN $p1",
		},
		{
			name:     "return_two_params",
			nodes:    []Node{&ReturnNode{Items: []interface{}{lit(1), lit(2)}}},
			expected: "RETURN $p1, $p2",
		},
		{
			name:     "limit_param",
			nodes:    []Node{&LimitNode{Expression: lit(10)}},
			expected: "LIMIT $p1",
		},
		{
			name:     "skip_param",
			nodes:    []Node{&SkipNode{Amount: lit(5)}},
			expected: "SKIP $p1",
		},
		{
			name:     "where_eq",
			nodes:    []Node{&WhereNode{Conditions: []Expression{rawExpr("$p1 = $p2")}}},
			expected: "WHERE $p1 = $p2",
		},
		{
			name:     "where_and",
			nodes:    []Node{&WhereNode{Conditions: []Expression{rawExpr("($p1 > $p2 AND $p3 < $p4)")}}},
			expected: "WHERE ($p1 > $p2 AND $p3 < $p4)",
		},
		{
			name: "unwind_return",
			nodes: []Node{
				&UnwindNode{Expression: lit([]int{1, 2}), AliasName: "x"},
		...
		{
			name:     "where_const_eq",
			nodes:    []Node{&WhereNode{Conditions: []Expression{rawExpr("10 = 10")}}},
			expected: "WHERE 10 = 10",
		},
		{
			name:     "where_const_and",
			nodes:    []Node{&WhereNode{Conditions: []Expression{rawExpr("(42 > 21 AND 3 < 7)")}}},
